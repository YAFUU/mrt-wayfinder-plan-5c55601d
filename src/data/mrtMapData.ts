...
function pinkDiagram(order: number) {
  ...
  if (order === 10) return diagramPoint(5.44, 5.95);
  if (order <= 16) return diagramPoint(5.44 + (order - 10) * 0.5, 5.37 - (order - 10) * 0.18);
  return diagramPoint(8.44 + (order - 17) * 0.2, 4.15 - (order - 17) * 0.36);
}
...
function segmentFromIds(stations: MrtMapStation[], ids: string[]) {
  const byId = new Map(stations.map((station) => [station.stationId, station]));
  return ids
    .map((id) => byId.get(id))
    .filter((station): station is MrtMapStation => Boolean(station));
}

function mapSegmentsFor(lineId: LineId, stations: MrtMapStation[]): MrtMapStation[][] {
  if (lineId !== "pink") return [stations];
  ...
  return [segmentFromIds(stations, mainLineIds), segmentFromIds(stations, ["PK09", "PK10"])];
}
...
export const MRT_MAP_LINES: MrtMapLine[] = LINES.map((line) => {
  ...
  return {
    ...
    status: line.status,
    stations,
    mapSegments: mapSegmentsFor(line.id, stations),
  };
});
