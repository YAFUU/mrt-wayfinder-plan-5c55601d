...
import { getStation, haversineMeters } from "@/services/routeService";
...
function validateMapData() {
  ...
  const expectedPinkStations = new Map([
    ["PK09", "Si Rat"],
  ...
  MRT_STATIONS.forEach((station) => {
    codeCounts.set(station.code, (codeCounts.get(station.code) ?? 0) + 1);
  });
  codeCounts.forEach((count, code) => {
    if (count > 1) console.warn("[MRT map] duplicate station code:", code);
  });
  expectedPinkStations.forEach((expectedName, code) => {
    const station = MRT_STATIONS.find((candidate) => candidate.code === code);
    if (!station || station.nameEn !== expectedName) {
      console.warn("[MRT map] unexpected pink line station mapping:", {
        code,
        expectedName,
        actual: station,
      });
  ...
  MRT_LINES.forEach((line) => {
    ...
    const segments = line.mapSegments?.length ? line.mapSegments : [line.stations];
    segments.forEach((segment, segmentIndex) => {
      segment.forEach((station, stationIndex) => {
        if (!station) console.warn("[MRT map] line segment references a missing station:", line.id);
        const nextStation = segment[stationIndex + 1];
        if (line.id === "pink" && nextStation) {
          const meters = haversineMeters(station, nextStation);
          if (meters > 8000) {
            console.warn("[MRT map] unusually long pink line segment:", {
              segmentIndex,
              from: station.code,
              to: nextStation.code,
...
function mrtLineFeatures(
  ...
  return {
    type: "FeatureCollection",
    features: MRT_LINES.filter((line) => visible[line.id] && line.stations.length > 1).flatMap(
      (line) => {
        const segments = line.mapSegments?.length ? line.mapSegments : [line.stations];
        return segments
          ...
          .map((segment, segmentIndex) => ({
            ...
            geometry: {
              type: "LineString",
              coordinates: segment.map((station) => lngLatFor(station, viewMode)),
            },
          }));
      },
    ),
