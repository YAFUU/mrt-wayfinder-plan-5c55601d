...
import { ArrowRightLeft, ArrowRight, Save, Share2, Map as MapIcon, Ticket as TicketIcon, Timer, Route as RouteIcon } from "lucide-react";
...
function RoutePage() {
  ...
  if (!result) {
    return (
      <div className="p-4 lg:p-8 max-w-3xl mx-auto">
        <PageHeader title={t("route.title")} />
        <EmptyState title={t("route.noRoute")} action={<Button asChild variant="outline"><Link to="/search">{t("common.search")}</Link></Button>} />
      </div>
    );
  }
  ...
  const prefBtn = (p: RoutePreference, label: string) => (
    <Button key={p} size="sm" variant={trip.preference === p ? "default" : "outline"} onClick={() => trip.setPreference(p)}>{label}</Button>
  );

  const buy = () => {
    if (!result.fareAvailable) { toast.error(t("route.fareUnavailable")); return; }
    nav({ to: "/checkout" });
  };
  ...
  const save = () => {
    storage.addSavedTrip({
      ...
      nickname: `${origin.nameTh} → ${destination.nameTh}`,
      icon: "⭐",
      originStationId: origin.id,
      destinationStationId: destination.id,
      createdAt: new Date().toISOString(),
    });
    toast.success("บันทึกเส้นทางประจำแล้ว");
  };
  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto space-y-4">
      <PageHeader
        title={`${origin.nameTh} → ${destination.nameTh}`}
        subtitle={`${result.totalStations} ${t("route.stations")} · ~${totalMin} ${t("common.min")} · ${result.transfers} ${t("route.transfers")}`}
        action={<Button variant="ghost" size="icon" onClick={trip.swap} aria-label={t("route.swap")}><ArrowRightLeft className="size-4" /></Button>}
      />
      ...
      <Card className="p-4">
        <h3 className="text-sm font-semibold flex items-center gap-2 mb-3"><RouteIcon className="size-4" /> Timeline</h3>
        <ol className="relative pl-4">
          {result.segments.map((seg, si) => {
            const line = LINES.find((l) => l.id === seg.lineId)!;
            return (
              <li key={si} className="relative mb-4">
                <div className="absolute -left-4 top-0 bottom-0 w-1" style={{ background: line.color }} />
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded" style={{ background: line.color, color: "white" }}>{line.code}</span>
                  <span className="text-xs text-muted-foreground">{seg.stations.length - 1} {t("route.stations")}</span>
                </div>
                <ul className="space-y-1">
                  {seg.stations.map((sid, i) => {
                    const s = getStation(sid)!;
                    const isTransfer = i === seg.stations.length - 1 && si < result.segments.length - 1;
                    return (
                      <li key={sid} className="flex items-center gap-2 text-sm">
                        <span className="size-2 rounded-full" style={{ background: line.color }} />
                        <span className={i === 0 || i === seg.stations.length - 1 ? "font-medium" : ""}>{s.nameTh}</span>
                        {isTransfer && <span className="text-[10px] text-warning uppercase">{t("route.changeAt")}</span>}
                      </li>
                    );
                  })}
                </ul>
      ...
      {suggestedExits.length > 0 && (
        <Card className="p-4">
          <h3 className="text-sm font-semibold mb-2">{t("exit.recommended")} · {destination.nameTh}</h3>
          <div className="space-y-2">
            {suggestedExits.map((e) => (
              <div key={e.exitCode} className="flex items-start gap-2 text-sm">
                <span className="grid place-items-center size-6 rounded bg-primary text-primary-foreground text-xs font-bold shrink-0">{e.exitCode}</span>
      ...
      {suggestedExits.length === 0 && (
        <DemoDisclaimer tone="warn">{t("exit.unavailable")} · {destination.nameTh}</DemoDisclaimer>
      )}

      {nearbyDest.length > 0 && (
        <Card className="p-4">
          <h3 className="text-sm font-semibold mb-2">สถานที่ใกล้ปลายทาง</h3>
          <div className="space-y-1 text-sm">
            {nearbyDest.map((p) => (
              <div key={p.id} className="flex items-center justify-between">
                <span>{p.nameTh}</span>
                <span className="text-xs text-muted-foreground">
                  {p.suggestedExit && `Exit ${p.suggestedExit} · `}{p.walkingMeters}ม.
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button onClick={buy} disabled={!result.fareAvailable} className="gap-1"><TicketIcon className="size-4" /> {t("route.buyTicket")}</Button>
        <Button variant="outline" onClick={save}><Save className="size-4 mr-1" /> {t("route.savedTrip")}</Button>
        <Button variant="outline" onClick={share}><Share2 className="size-4 mr-1" /> {t("route.shareRoute")}</Button>
        <Button variant="outline" asChild><Link to="/map"><MapIcon className="size-4 mr-1" /> {t("route.viewOnMap")}</Link></Button>
        <Button variant="ghost" asChild><Link to="/queue"><Timer className="size-4 mr-1" /> {t("nav.queue")}</Link></Button>
      </div>
